	...
	if(trim($_POST['user']['email']) == '' || $_POST['user']['password'] == '') {
		...
		try {			
            ...
            if($_POST['payment_method']=="credit_card"){
                ...
                if(trim($_POST['user']['paypal']['name']) != ""){                     
                    $userId = addUser($_POST['user']['email'], $_POST['user']['password']); 
                    savePayPal($_POST['user']['paypal'],$userId);
                }
            }
...
<html lang='en'>
  ...
  <body>
    ...
    <div class='container' id='content'>
      ...
      <h2>Sign up</h2>
	  <p>Sign up for a new RedQ Team account.</p>
      ...
      <form accept-charset="UTF-8" action="./sign_up.php" autocomplete="off" class="simple_form form-horizontal new_user" id="new_user" method="post" novalidate="novalidate"><div style="margin:0;padding:0;display:inline"><input name="utf8" type="hidden" value="&#x2713;" /><input name="authenticity_token" type="hidden" value="vpVuNuIt9fRZzLm0eE0gk4h249k0nZPB/WEXWn9ETwg=" /></div>
        ...
        <div class="control-group name required"><label class="name required control-label" for="name"><abbr title="required">*</abbr> Select your payment method: </label>
            <div class="controls">
                <input autofocus="autofocus" class="string email required" class="payment_method" name="payment_method" type="radio" value="paypal" />PayPal
                <input autofocus="autofocus" class="string email required" class="payment_method" name="payment_method" type="radio" value="credit_card"/>Credit Card
            </div>
        </div>
        <div id="paypal">
            <legend>Add PayPal (Optional)</legend>
            <p>Provide information bellow which is stored safely with PayPal.</p>
            <div class="control-group name required"><label class="name required control-label" for="name"><abbr title="required">*</abbr> Name</label>
                <div class="controls">
                    <input autofocus="autofocus" class="string email required" id="name" name="user[paypal][name]" size="50" type="text" value="rubel" placeholder="name"/>
                </div>
            </div>
            <div class="control-group name required"><label class="address required control-label" for="address"><abbr title="required">*</abbr> Address</label>
                <div class="controls">
                    <input autofocus="autofocus" class="string email required" id="address" name="user[paypal][address]" size="200" type="text" value="khilkhet" placeholder="address"/>
                </div>
            </div>
            <div class="control-group name required"><label class="city required control-label" for="city"><abbr title="required">*</abbr> City</label>
                <div class="controls">
                    <input autofocus="autofocus" class="string email required" id="city" name="user[paypal][city]" size="50" type="text" value="dhaka" placeholder="city"/>
                </div>
            </div>
        </div>
