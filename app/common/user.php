...
function updateUserInfo($userId, $payerId) {

	if($userId == NULL && $payerId == NULL) {
		return;
	}
	$conn = getConnection();
	$args = array(USERS_TABLE); $updates = array();
	if($userId != NULL) {
		$args[] = mysql_real_escape_string($userId);
		$updates[] = "user_id='%s'";
	}
	if($payerId != NULL) {
		$args[] = mysql_real_escape_string($payerId);
		$updates[] = "payer_id='%s'";
	}
	$args[] = mysql_real_escape_string($userId);
	
	$query = vsprintf("UPDATE %s SET " . implode(', ', $updates) . " WHERE user_id='%s'", $args);	
	$result = mysql_query($query, getConnection());
	if(!$result) {
		$errMsg = "Error updating user record: " . mysql_error($conn);
		mysql_close($conn);
